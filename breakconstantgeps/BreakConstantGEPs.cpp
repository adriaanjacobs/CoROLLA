...
{
    if (ConstantExpr * CE = llvm::dyn_cast<ConstantExpr>(V))
    {
...
convertGEP (ConstantExpr * CE, Instruction * InsertPt)
{
    ...
    return (GetElementPtrInst::Create (llvm::cast<llvm::GEPOperator>(CE)->getSourceElementType(),
                                        CE->getOperand(0),
                                        arrayIdices,
                                        CE->getName(),
                                        InsertPt));
...
BreakConstantGEPs::runOnModule (Module & module)
{
    ...
    for (Module::iterator F = module.begin(), E = module.end(); F != E; ++F)
    {
        ...
        while (Worklist.size())
        {
            ...
            //
            if (PHINode * PHI = llvm::dyn_cast<PHINode>(I))
            {
